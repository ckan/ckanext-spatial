...
class CSWHarvester(SpatialHarvester, SingletonPlugin):
    ...
    def get_original_url(self, harvest_object_id):
        ...
        url = urlunparse(
            (parts.scheme, parts.netloc, parts.path, None, urlencode(params), None)
        )
    ...
    def gather_stage(self, harvest_job):
        ...
        try:
            ...
            self._save_gather_error(
                "Error contacting the CSW server: %s" % e, harvest_job
            )
            return None
        ...
        try:
            for identifier in self.csw.getidentifiers(
                ...
                try:
                    ...
                    if identifier is None:
                        log.error(
                            "CSW returned identifier %r, skipping..." % identifier
                        )
                        continue
            ...
            self._save_gather_error(
                "Error gathering the identifiers from the CSW server [%s]" % str(e),
                harvest_job,
            )
            return None
        ...
        for guid in new:
            obj = HarvestObject(
                guid=guid, job=harvest_job, extras=[HOExtra(key="status", value="new")]
            )
            obj.save()
            ids.append(obj.id)
        for guid in change:
    ...
    def fetch_stage(self, harvest_object):
        ...
        try:
            ...
            self._save_object_error(
                "Error contacting the CSW server: %s" % e, harvest_object
            )
            return False

        identifier = harvest_object.guid
        try:
            record = self.csw.getrecordbyid(
                [identifier], outputschema=self.output_schema()
            )
        except Exception as e:
            self._save_object_error(
                "Error getting the CSW record with GUID %s" % identifier, harvest_object
            )
            return False

        if record is None:
            self._save_object_error(
                "Empty record for GUID %s" % identifier, harvest_object
            )
            return False
